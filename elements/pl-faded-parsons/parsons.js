(function($, _) { // wrap in anonymous function to not show some helper variables
  ...
  class LineBasedGrader {
    ...
    grade(elementId) {
      ...
      var lisStudentCodeLineObjects = studentCodeLineObjects.filter(function (lineObject) { return !lineObject.lisIgnore; });
      var inv = LIS.best_lise_inverse_indices(lisStudentCodeLineObjects
  ...
  // Creates a parsons widget. Init must be called after creating an object.
   class ParsonsWidget {
    ...
    parseCode(lines, max_distractors) {
      var distractors = [], given = [], indented = [], widgetData = [], lineObject, errors = [], that = this;
    ...
    getHash(searchString) {
      var hash = [], ids = $(searchString).sortable('toArray'), line;
    ...
    codeLineToHTML(codeline) {
      ...
      while (codeline.code.search(/!BLANK/) >= 0) {
        ...
        const inputFieldName = codeline.id.toString() + '-' + numBlanksThisLine.toString();
